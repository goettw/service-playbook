...
<div class="container">
	<div class="row">
		<form:form method="POST" action="${addUrl}" commandName="profile">
			...
			<div class="control-group">
				<label for="remember"><spring:message code="remember" /></label> 
				<input type="checkbox" id="remember" name="_spring_security_remember_me" value="true" />
			</div>
			<div class="control-group">
				<input type="submit" value="Login" name="action" class="btn btn-default btn-xs" />
			</div>
		</form:form>
	</div>
	<div class="row">
		<c:if test="${param.login_error != null}">
			<spring:message code="loginFailure" />
			<p>
